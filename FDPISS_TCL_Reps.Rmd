...
IDENTITY <- read.delim("E:/FEO/Replicate MOFO analysis/Combodeal/IDENTITY.txt", header=FALSE)

Group = factor(IDENTITY$V1)
...
Bounddata<- cbind(SCMYO[1:14], TCL1[2:385])
#write.table(Bounddata,file="Bounddata.txt",sep="\t", row.names = FALSE)
...
Landmark_Genes_n978 <- read.delim("E:/FEO/Replicate MOFO analysis/Landmark_Genes_n978.txt", dec=",")
GGG <- tolower(Landmark_Genes_n978$Gene.Symbol)
YYY<-gsub("(^|[[:space:]])([[:alpha:]])", "\\1\\U\\2", GGG, perl=TRUE)



plot.data = dataALL2[row.names(dataALL2) %in% YYY,]

y <- DGEList(counts=plot.data, genes=row.names(plot.data))  
y = calcNormFactors(y)
...
mds<- plotMDS(y, top=100, gene.selection="common",dim.plot=c(1,5), cex=.7)
```




#Load up L1000 list and intersect with data before MDS
